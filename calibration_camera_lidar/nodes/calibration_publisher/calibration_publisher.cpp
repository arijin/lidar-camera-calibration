...
void tfRegistration(const cv::Mat &camExtMat, const ros::Time &timeStamp)
{
    ...
    std::cout << camera_id_str << std::endl;
...
int main(int argc, char *argv[])
{
	...
	private_nh.param<std::string>("target_frame", target_frame_, "rslidar"); //default is robosense
	...
	ROS_INFO("name_space_str: %s", name_space_str.c_str());
	camera_id_str = "camera";
	//if (name_space_str != "/")
	//{
		image_topic_name = "/mindvision" + image_topic_name;
